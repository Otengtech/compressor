      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col lg:flex-row gap-8 lg:gap-12 items-stretch">
          ...
          >
            <div>
              {/* Quote */}
              <div className="mb-6">
                <svg className="w-10 h-10 text-lime-300" fill="currentColor" viewBox="0 0 32 32">
              ...
              <div className="flex items-center gap-2 mb-4">{renderStars(currentData.rating)}</div>
              ...
              <div className="flex items-center gap-4">
                <div className="relative">
                  <div className="w-16 h-16 rounded-full overflow-hidden bg-lime-100">
                    {currentData.avatar ? (
                      <img src={currentData.avatar} alt="" className="w-full h-full object-cover" />
                    ) : (
                ...
                <div>
                  <h3 className="font-bold text-gray-900 text-lg">{currentData.name}</h3>
            ...
            <div className="hidden lg:flex justify-center items-center gap-4">
              <button onClick={handlePrev} className="p-2 rounded-full hover:bg-lime-50">
              ...
              <span className="text-gray-600 text-sm">{currentTestimonial + 1} / {testimonials.length}</span>

              <button onClick={handleNext} className="p-2 rounded-full hover:bg-lime-50">
                <svg className="w-6 h-6 text-gray-600" fill="none" stroke="currentColor"
                  viewBox="0 0 24 24">
          ...
          >
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4 h-full">
              {images.map((image) => (
                <div key={image.id} className="rounded-xl overflow-hidden shadow-md hover:scale-105 transition-all duration-300">
                  <img src={image.img} alt="" className="w-full h-full object-cover" loading="lazy" />
                </div>
              ))}
            </div>
          </div>
        ...
        >
          <button onClick={handlePrev} className="p-3 rounded-full bg-lime-100 hover:bg-lime-200">
            <svg className="w-5 h-5 text-lime-700" fill="none" stroke="currentColor"
              viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7"/>
            </svg>
          </button>
